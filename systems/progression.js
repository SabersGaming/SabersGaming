// ============================================================
// systems/progression.js — UNLOCK & GRIMOIRE SYSTEM (Pillar II)
// Owns: all unlock conditions, grimoire skill purchase,
//       grimoire prestige, quest checking.
// Reads state. Writes unlock flags to state.
// NEVER generates resources. NEVER manipulates combat directly.
// SINGLE SOURCE OF TRUTH for what the player has access to.
// ============================================================
'use strict';
...
function buyGrimoireSkill(skillId) {
  ...
  if (sk.tier > 1) {
    ...
    if (prevOwned < 4) {
      showStoryModal("Forbidden", `You need 4 skills from Tier ${sk.tier - 1} before unlocking Tier ${sk.tier}. You have ${prevOwned}.`);
      return;
    }
  }

  // Final Ascension gate
  ...
  for (const conflict of sk.conflictsWith) {
    if (gs.grimoireSkills[conflict]) {
      showStoryModal("Malakar RECOILS", `Conflicts with ${CONFIG.GRIMOIRE_SKILLS[conflict].name}. Some dark powers cannot coexist.`);
      return;
    }
  }
...
// Grimoire Prestige — Pillar IV: must call full state reset
function performGrimoirePrestige() {
  ...
  // Full run reset (tomes and caps PERSIST — this is intentional grimoire design)
  gs.soulEssence = 0;
  gs.darkMana    = 0;
  gs.corpseMatter = 0;
  ...
  gs.tower = { floorsBuilt: 0, maxFloorEver: gs.tower.maxFloorEver, isBuilding: false, buildFloor: 0, buildTimeRemaining: 0, buildTimeFull: 0 };
  ...
  gs.statistics.currentRun = { soulEssenceEarned: 0, darkManaEarned: 0, corpseMatterEarned: 0, unitsLost: 0, zombiesRaised: 0, ghoulsRaised: 0, golemsRaised: 0 };
  gs.grimoirePrestigePendingSkills = [];
  gamePaused = false;
...
// ── Quest System ──────────────────────────────────────────────
// Isolated in progression.js per the Constitution.
var _questsCompleted = {};

function checkQuests() {
  const gs = gameState;
  if (!gs._questsCompleted) gs._questsCompleted = {};
  for (const q of CONFIG.QUESTS) {
    if (gs._questsCompleted[q.id]) continue;
    const lifetime = gs.statistics.lifetime;
    const val = lifetime[q.targetStat] || 0;
    if (val >= q.target) {
      gs._questsCompleted[q.id] = 'claimable';
      addLog(`QUEST COMPLETE: "${q.name}" — claim your reward!`, 'quest');
    }
  }
}
...
function claimQuest(questId) {
  const gs = gameState;
  if (!gs._questsCompleted) gs._questsCompleted = {};
  if (gs._questsCompleted[questId] !== 'claimable') return;
  const q = CONFIG.QUESTS.find(x => x.id === questId);
  if (!q) return;
  gs._questsCompleted[questId] = 'claimed';
  gs.tomeUpgradePoints += q.reward;
  addLog(`Quest claimed: "${q.name}" — +${q.reward} Tome Upgrade Points!`, 'quest');
}
